...
#define TIMEOUT_US 50000   // 50 ms timeout for Task 1
...
void *client_thread_func(void *arg) {
    ...
    while (data->rx_cnt + data->lost_cnt < num_requests) {
        ...
        while (data->in_flight < data->window_size &&
            ...
            int idx = pkt.seq % data->window_size;
            data->window[idx].seq = pkt.seq;
        ...
        for (int i = 0; i < n_events; i++) {
            if (events[i].data.fd == data->socket_fd) {
                ...
                recvfrom(data->socket_fd, &ack_pkt, sizeof(ack_pkt), 0,
                         (struct sockaddr *)&data->server_addr,
                         &addrlen);

                int idx = ack_pkt.seq % data->window_size;
        ...
        // -----------------------------
        // 3. TIMEOUT PHASE (Task 1)
        // -----------------------------
        now = now_us();
        for (int i = 0; i < data->window_size; i++) {
            if (!data->window[i].acked &&
                (now - data->window[i].send_time) > TIMEOUT_US) {
...
void run_client() {
    ...
    for (int i = 0; i < num_client_threads; i++) {
        ...
        thread_data[i].window_size = 10;
        thread_data[i].tx_cnt = 0;
        thread_data[i].rx_cnt = 0;
        thread_data[i].lost_cnt = 0;
        thread_data[i].next_seq = 0;
        thread_data[i].in_flight = 0;
